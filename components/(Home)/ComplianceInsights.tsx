...
export default function ComplianceInsights() {
    ...
    return (
        <section className="w-full bg-[#F8F9FA] pt-16 pb-16 md:py-24 px-6 ">
            <div className="max-w-[1620px] mx-auto">

                <h2 className="font-Inter font-semibold text-[28px] sm:text-[34px] md:text-[40px] leading-[125%] text-[#1E374F] mb-12 text-left">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-y-6 gap-x-4">
                    {insights.map((item) => (
                        <div
                            key={item.id}
                            className="flex flex-col sm:flex-row items-start gap-4 w-full text-left"
                        >
                            <div className="w-full sm:w-50 md:w-40 xl:w-48.25 aspect-square relative rounded-[25px] overflow-hidden bg-[#ECEBEC] shrink-0 shadow-sm mx-auto sm:mx-0">
                                <Image
                                    src={item.imageSrc}
                                    alt={item.imageAlt}
                                    fill
                            ...
                            <div className="flex flex-col flex-1 justify-between sm:min-h-50 md:min-h-40 xl:min-h-48.25 py-1 w-full">
                                <div>
                                    <span className="inline-block bg-[#333333] text-white font-Inter font-medium text-[14px] px-3 py-1 rounded-[10px] mb-3">
                                        {item.badgeText}
                                    </span>

                                    <h3 className="font-Inter font-semibold text-[18px] xl:text-[20px] leading-[135%] text-[#1E374F] mb-2">
                                        {item.title}
                                    </h3>
                                </div>
                                ...
                                <div className="mt-3 sm:mt-auto">
                                    <Link
                                        href={item.linkHref}
                                        className="inline-block font-Inter font-semibold text-[16px] text-[#4B90CD] hover:text-[#3577b5] transition-colors border-b-2 border-[#4B90CD]/30 hover:border-[#4B90CD] pb-0.5"
                                    >
                                        {item.linkText}
                                    </Link>
                                </div>
